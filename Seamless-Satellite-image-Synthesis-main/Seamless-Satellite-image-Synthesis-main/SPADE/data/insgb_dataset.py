...
class INSGBDataset(BaseDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        # coord_row_major now defined globally in base_options; keep logic but avoid duplicate argparse registration
    ...
    def initialize(self, opt):
        ...
        # Optional reordering: convert x_y.png ordering (column-major after natural sort) into row-major (y,x)
        if opt.coord_row_major:
            def reorder(paths):
                def parse_xy(p):
                ...
                for path in paths:
                    xy = parse_xy(path)
                    if xy is not None:
                        with_xy.append((xy[0], xy[1], path))
                if not with_xy:
                    return paths  # nothing to reorder
                # Sort by y then x for row-major iteration
                with_xy.sort(key=lambda tup: (tup[1], tup[0]))
                return [p for _, _, p in with_xy]

            label_paths = reorder(label_paths)
            image_paths = reorder(image_paths)
            if not opt.no_instance:
                instance_paths = reorder(instance_paths)
            cg_paths = reorder(cg_paths)
